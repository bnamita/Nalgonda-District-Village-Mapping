  ...
  createAttrList: function() {
    ...
    d3.csv('./data/metadata/census_fields_meta.csv', function(csv_metadata) {
      ...
      for (var i = 0; i < csv_metadata.length; i++) {
        if (
          csv_metadata[i]['column'].indexOf(
            '(If not available within the village'
          ) === -1
        ) {
      ...
      for (i = 0; i < colArr.length; i++) {
        selectHTML +=
          "<option value='" + colArr[i] + "'>" + colArr[i] + '</option>';
      }
  ...
  attachEvents: function() {
    ...
    $('#district-list').on('change', function(val) {
      ...
      if (self.isCustom) {
        ...
        $('#download_geojson').attr(
          'href',
          'geometry/' + geojson_file_map[self.district_name] + '.geojson'
        );
      }
    });
    ...
    self.districtMap.setFieldName('Total Population of Village');
