...
DistrictMap_Leaflet.prototype = {
  setup: function(args) {
    ...
    this.district_name = args.district_name || 'Nalgonda';
  ...
  setup_map: function() {
    ...
    var MBAttrib =
      '&copy; ' +
      osmLink +
      ' Contributors & <a href="https://www.mapbox.com/about/maps/">Mapbox</a>';
    MBAttrib +=
      '<br>Link to census data: ' +
      '<a href="http://www.censusindia.gov.in/2011census/dchb/DCHB.html">Click here</a>';
    ...
    if (categorical_variables.indexOf(this.field_name) !== -1) {
      ...
      for (var i = 0; i < grades.length; i++) {
        if (Math.floor(grades[i] * max) !== Math.floor(grades[i + 1] * max)) {
          div.innerHTML +=
            '<i style="background:' +
            this.getLegendColor(grades[i], max) +
            '"></i> ' +
            /*Math.round(grades[i]*max) + */ (grades[i + 1] * max
              ? Math.floor(grades[i + 1] * max) + '<br>'
              : Math.floor(max));
        }
      }
      div.innerHTML += '<br> <i style="background:#000"></i> Data NA <br>';
      ...
      for (var i = 0; i < grades.length; i++) {
        if (Math.round(grades[i] * max) !== Math.round(grades[i + 1] * max)) {
          div.innerHTML +=
            ...
            this.getLegendColor(grades[i], max) +
            '"></i> ' +
            this.easynumber(grades[i] * max) +
            (grades[i + 1] * max
              ? '&ndash;' + this.easynumber(grades[i + 1] * max) + '<br>'
              : '&ndash;' + this.easynumber(max));
        }
      }
      ...
      this._div.innerHTML =
        ...
        (shapeLayerData
          ? '<span class="info-value"><b>' +
            ...
            '<br /> (Village Code: ' +
            shapeLayerData[fieldToMatch['geometry']] +
            ')<br/>'
          : '');
      if (
        csvLayerData &&
      ...
      self.choroLayer = L.geoJson(null, {
        style: function(feature) {
          ...
          var csvLayerData = _.find(self.featureData, function(d) {
            return (
              d[self.fieldToMatch] ==
              feature.properties[fieldToMatch['geometry']]
            );
          });
          return {
            weight: 2,
          ...
          if (self.searchControl === undefined) {
            ...
            self.searchControl
              ...
              .on('search:collapsed', function(e) {
                self.choroLayer.eachLayer(function(layer) {
  ...
  parseCSVFile: function() {
    ...
    var filepath =
      'data/csvdata/census_split_by_district/' +
      geojson_file_map[self.district_name] +
      '.csv';
    Papa.parse(filepath, {
      download: true,
      header: true,
  ...
  updateStyle: function() {
    ...
    this.choroLayer.eachLayer(function(layer) {
      ...
      var csvLayerData = _.find(self.featureData, function(d) {
        return (
          d[self.fieldToMatch] ==
          layer.feature.properties[fieldToMatch['geometry']]
        );
      });
